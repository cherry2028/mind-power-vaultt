...
const css = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .bg::after{content:'';position:absolute;top:-50%;left:-60%;width:35%;height:200%;background:rgba(255,255,255,0.15);transform:skewX(-20deg);transition:left 0.55s ease}
  .bg:hover{transform:translateY(-3px);box-shadow:0 10px 40px rgba(201,168,76,0.42)}
  .bg:hover::after{left:130%}

  /* ── Outline button ── */
  .bo{cursor:pointer;transition:all 0.28s ease}
  .bo:hover{background:rgba(201,168,76,0.08)!important;border-color:rgba(201,168,76,0.7)!important;transform:translateY(-1px)}

  /* ── Page in animation ── */
  .pin{animation:pageIn 0.6s cubic-bezier(.4,0,.2,1) forwards}
  @keyframes pageIn{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:none}}

  /* ── Keyframes ── */
  @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-12px)}}
  @keyframes breathe{0%,100%{opacity:0.06}50%{opacity:0.18}}
  @keyframes scr{0%,100%{opacity:0.3;transform:translateY(0)}50%{opacity:0.9;transform:translateY(8px)}}
  @keyframes ritIn{from{opacity:0;letter-spacing:8px}to{opacity:1;letter-spacing:normal}}
  @keyframes lineG{from{height:0;opacity:0}to{height:56px;opacity:1}}
  ...
  @keyframes orb2{0%,100%{transform:translate(0,0) scale(1)}33%{transform:translate(-50px,60px) scale(1.05)}66%{transform:translate(40px,-30px) scale(1.08)}}
  ...
  .rit{animation:ritIn 0.9s ease forwards}
  ...
  /* ── Stagger animations ── */
  .s1{animation:pageIn 0.6s ease 0.1s both}
  .s2{animation:pageIn 0.6s ease 0.22s both}
  .s3{animation:pageIn 0.6s ease 0.34s both}
  .s4{animation:pageIn 0.6s ease 0.46s both}
  .s5{animation:pageIn 0.6s ease 0.58s both}

  /* ── Mesh grid ── */
  .mesh{animation:meshMove 12s ease-in-out infinite}

  /* ── Responsive ── */
  @media(max-width:600px){
    .wa-float{bottom:20px;right:16px;width:50px;height:50px;font-size:22px}
  }
`;
...
export default function MPV() {
  ...
  const Hero = () => (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", position: "relative", overflow: "hidden", background: "radial-gradient(ellipse at 30% 20%, rgba(201,168,76,0.07) 0%, transparent 50%), radial-gradient(ellipse at 75% 80%, rgba(150,100,20,0.05) 0%, transparent 50%), #04040C" }}>
      ...
      <div className="scrl" style={{ position: "absolute", bottom: 32, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
        <div style={{ width: 1, height: 40, background: `linear-gradient(to bottom,transparent,${G.gold}60)` }} />
        <span style={{ fontSize: 7, letterSpacing: 5, color: `${G.gold}45`, textTransform: "uppercase", fontFamily: sans }}>scroll</span>
      </div>
    </div>
  );
...
━━━━━━━━━━━━━━━━━━━━━`
  ...
  return (
    <div style={{ background: G.black, color: G.smoke, fontFamily: sans, minHeight: "100vh", overflowX: "hidden" }}>
      ...
      {/* Floating WhatsApp Button */}
      {phase > 0 && (
        <button className="wa-float" onClick={() => window.open(`https://wa.me/${CHERRY_WA}?text=${waMsg}`, "_blank")} title="Chat on WhatsApp" aria-label="WhatsApp">
