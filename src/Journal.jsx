...
export default function Journal({ lang, onBack }) {
  ...
  const checkAccess = async () => {
    ...
    try {
      ...
      if (result.valid) {
        ...
        alert(result.error || "Invalid Access Code. Please contact K Prasad.");
      }
    } catch (error) {
  ...
  const addStudent = (name, city, phone) => {
    const code = "MPV-" + window.crypto.getRandomValues(new Uint32Array(1))[0].toString(36).toUpperCase().substring(0, 6);
