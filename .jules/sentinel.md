...
## 2025-02-24 - [Missing Rate Limiting on Lead API]
**Vulnerability:** The public-facing endpoint `api/save-lead.js`, which consumes the Telegram API via bot credentials, was lacking any rate-limiting logic. This allowed an unauthenticated attacker to flood the endpoint, potentially causing Denial of Wallet (DoW) attacks or exhausting Telegram API quota, leading to disrupted notifications.
**Learning:** Endpoints that proxy to external services or act as form submissions must explicitly integrate shared rate limiting (e.g., `checkSimpleLimit`), even if they appear lower priority than notification/order endpoints. The `save-lead.js` also suffered from missing CORS headers, leading to potential client-side integration issues.
**Prevention:** Any new API handler in `api/` exposing POST logic to client requests—especially those using `process.env` credentials for third parties—must include at minimum `checkSimpleLimit` and proper CORS setup.
